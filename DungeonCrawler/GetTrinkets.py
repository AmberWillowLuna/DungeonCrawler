...
def GetTrinket(screen, player1):
    ...
    TakeButton = button.Button(
        800 * scale, 600 * scale, 300 * scale, 100 * scale,
        "Take", (0, 100, 0), (0, 200, 0)
    )

    # Button to skip the trinket
    SkipButton = button.Button(
        800 * scale, 750 * scale, 300 * scale, 100 * scale,
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if TakeButton.is_clicked(mouse_pos, event):
                    ...
                    player1.PickUp(trinket)
                    trinket.apply_effect(player1)
                    running = False
        ...
        for i, line in enumerate(desc_lines):
            ...
            screen.blit(desc_text, (500 * scale, 300 * scale + i * 40 * scale))

        # Draw the trinket's button (optional, for visual representation)
        trinket_button = button.Button(
            600 * scale, 350 * scale, 800 * scale, 200 * scale,
            trinket.name, (255, 191, 0), (150, 150, 150)
        )
...
def wrap_text(text, max_width, font):
    ...
    for word in words:
        word_width = font.size(word)[0]  # Extract the width from the tuple
        ...
        if current_width + word_width <= max_width:
            ...
            current_line = [word]
            current_width = word_width + font.size(' ')[0]  # Reset width
